...
{
    Heap heapSort(size, array, comp, move);
    //check if sorted and store move and comp
    if(sorted(array, size))
    {
        info[sort][0] = move;
    ...
    {
        //error message
        std::cout << "The heap sort algorithm failed.\n"; 
    }
    sort++;
}
...
int main()
{
    ...
    //quick sort all arrays
    heap_sort(ARRAY_SIZE, tBST);
    heap_sort(ARRAY_SIZE, tAVG);
    heap_sort(ARRAY_SIZE, tWST);
