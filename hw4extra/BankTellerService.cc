...
BankTellerService::BankTellerService(std::string infS, std::string otfS)
{
    ...
    //BusyTeller = false; //teller 1 is free
    CustomersNum = 0; //no current customers

    //asks the user to input the number of bank tellers
    std::cout << "Enter the total number of tellers: ";
    std::cin >> tellerNum;

    //there are no busy tellers
    freeTellers = tellerNum; 
}
...
void BankTellerService::serveCustomers()
{
    ...
    while(!EventsQ.isEmpty())
    {
        ...
        if(nextEvent.EventType == 'A')
        {
            ...
            //if there are any tellers available
            if(freeTellers != 0)
            {
                freeTellers--; //decrement the number of available tellers
...
void BankTellerService::getStatistics()
{
    ...
    for(int i = 0; i < CustomersNum; i++)
    {
        ...
        //confirms that waitTime is not negative
        if(waitTime < 0)
            waitTime = 0;
